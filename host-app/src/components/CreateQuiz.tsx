...
const QuestionSchema = v.object({
  ...
  correctIndexes: v.pipe(v.array(v.number()), v.minLength(1, 'Select at least one correct answer')),
  timerSec: v.pipe(v.number(), v.minValue(5), v.maxValue(120)),
})

const FormSchema = v.object({
  title: v.pipe(v.string(), v.minLength(1, 'Title required')),
  questions: v.pipe(v.array(QuestionSchema), v.minLength(1, 'Add at least one question')),
})
...
const CHOICE_COLORS = ['var(--c-red)', 'var(--c-blue)', 'var(--c-amber)', 'var(--c-green)']
...
function CreateQuiz({ onSubmit }: CreateQuizProps) {
  ...
  return (
    <div className="phase-container" style={{ maxWidth: 700, textAlign: 'left' }}>
      <motion.h1
      ...
      <form className="create-form" onSubmit={handleSubmit(onFormSubmit)}>
        <div className="form-group">
          ...
          <input {...register('title')} placeholder="My Awesome Quiz" autoFocus />
          {errors.title && (
            <span style={{ color: 'var(--status-err)', fontSize: '0.8rem', marginTop: '0.25rem', display: 'block' }}>
            ...
            return (
              ...
              >
                <div className="question-card-header">
                  ...
                  {fields.length > 1 && (
                    <button type="button" className="btn-remove" onClick={() => remove(qIdx)}>
                      Remove
                    </button>
                  )}
                </div>
                ...
                <div className="form-group">
                  ...
                  />
                  {qErrors?.text && (
                    <span style={{ color: 'var(--status-err)', fontSize: '0.78rem' }}>{qErrors.text.message}</span>
                  )}
                </div>
                  ...
                  {([0, 1, 2, 3] as const).map(cIdx => (
                    <div key={cIdx} className="choice-input-group">
                          ...
                          <input
                            ...
                            onChange={e => {
                              const next = e.target.checked
                                ...
                                : (f.value as number[]).filter(i => i !== cIdx)
                              f.onChange(next)
                            }}
                          />
                        )}
                      ...
                      <input
                        ...
                        {...register(`questions.${qIdx}.choices.${cIdx}` as any)}
                        placeholder={`Choice ${cIdx + 1}`}
                        style={{ borderLeft: `3px solid ${CHOICE_COLORS[cIdx]}` }}
                      />
                    </div>
                  ))}
                </div>
                {qErrors?.correctIndexes && (
                  <span style={{ color: 'var(--status-err)', fontSize: '0.78rem' }}>
                    {qErrors.correctIndexes.message}
                  </span>
                )}

                <div className="form-group" style={{ marginTop: '0.75rem', maxWidth: 200 }}>
                  <label>Timer</label>
                  <select {...register(`questions.${qIdx}.timerSec`, { valueAsNumber: true })}>
